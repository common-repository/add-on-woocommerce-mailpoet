...
if ( ! class_exists( 'MPWA_Frontend_Fields' ) ) {
	class MPWA_Frontend_Fields {
		...
		public $show_gdpr_unsub;
		public $gdpr_unsub_label;
		public $show_gdpr_consent;
		public $multi_subscription;
		public $privacy_page;
		public $gdpr_consent_text;
		...
		private function __construct() {
			...
			if ( is_user_logged_in() ) {
				...
				$this->user_subscriber = Subscriber::whereEqual(
					'email',
					$current_user->user_email
				)->whereEqual( 'status', 'subscribed' )->findArray();

				// If logged in user is not mailpoet subscriber
				if ( empty( $this->user_subscriber ) ) {
					$this->run_actions();
				} elseif ( $this->user_subscriber[0]['source'] == 'wordpress_user' || $this->user_subscriber[0]['source'] == 'woocommerce_user' ) {
					$this->run_actions();
					if ( 'yes' == $this->show_gdpr_unsub && 'yes' == $this->show_form && ! empty( $this->user_subscriber[0]['confirmed_ip'] ) ) {
						add_action( 'woocommerce_' . $this->form_position, array( $this, 'show_unsubscribe_form' ) );
					}
				}
			} else {
				$this->run_actions();
		 ...
		 */
		public function show_unsubscribe_form() {
			?>
		...
		public function run_actions() {
			...
			if ( ! empty( $this->privacy_page ) ) {
				...
				if ( ! empty( $this->privacy_page ) ) {
					...
					$this->privacy_page = '<a href="' . $this->privacy_page . '" target="_blank">' . $this->__( 'Privacy Policy' ) . '</a>';
			...
			$this->list_ids = get_option( 'wc_mailpoet_segment_list', array() );
